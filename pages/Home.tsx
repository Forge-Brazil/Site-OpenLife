...
const Home: React.FC = () => {
  ...
  return (
    <div className="-mt-20 overflow-x-hidden">
      ...
      {/* HERO */}
      <section
        className="relative text-white min-h-screen flex flex-col items-center justify-center text-center px-6 pt-32 pb-20"
        style={{ background: 'linear-gradient(160deg, #3b0764 0%, #6B2D8B 45%, #a855f7 80%, #f3e8ff 100%)' }}
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full opacity-20"
            style={{ background: 'radial-gradient(circle, #F57C20 0%, transparent 70%)' }} />
        </div>
        <div className="relative z-10">
          <p className="text-orange-brand font-bold text-xs tracking-[0.25em] uppercase mb-5">
            OpenLife English School
          </p>
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tight leading-[0.9] mb-8 text-white">
            Fluência<br />em inglês.
          </h1>
          <p className="text-lg md:text-2xl mb-10 max-w-xl mx-auto leading-relaxed text-white/80">
      ...
      {/* METODOLOGIA */}
      <section className="text-white py-28 px-6" style={{ background: 'linear-gradient(180deg, #6B2D8B 0%, #3b0764 100%)' }}>
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-white/70 text-lg md:text-xl max-w-2xl mx-auto mb-12 leading-relaxed">
            Nossa metodologia ESL simula imersão total — sem precisar sair do Brasil.
          </p>
        ...
        <div className="max-w-5xl mx-auto mt-16 grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { title: 'Aulas ao Vivo', desc: 'Professores ESL, grupos reduzidos de até 8 alunos.' },
            ...
            <div key={i} className="bg-white/10 border border-white/15 rounded-2xl p-6 text-left">
      ...
      {/* CTA FINAL */}
      <section className="text-white py-28 px-6 text-center"
        style={{ background: 'linear-gradient(160deg, #6B2D8B 0%, #3b0764 60%, #1e0536 100%)' }}>
        <div className="max-w-3xl mx-auto">
